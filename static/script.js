...
async function predictModel1() {
    ...
    try {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            if (data.success !== false) {
                ...
                if (data.data && data.data.products && Array.isArray(data.data.products)) {
                    // Use actual product data returned by the layout + OCR pipeline
        ...
        // Show accuracy metrics panel
        const latestMetrics = invoiceResults.length ? invoiceResults[invoiceResults.length - 1].metrics : null;
        updateMetricsPanel(latestMetrics || null);
...
function updateMetricsPanel(metricsPayload) {
    ...
    if (!metricsPayload) {
        ...
        grid.innerHTML = '<div class="no-result">Metrics will appear after processing.</div>';
        return;
    }

    const invoiceMetrics = metricsPayload.invoice || {};
    const running = metricsPayload.running_average || {};
    const training = metricsPayload.layout_training || {};

    const cards = [];

    if (invoiceMetrics.layout_confidence !== undefined) {
        cards.push({
            label: 'Layout Confidence (invoice)',
            value: formatPercent(invoiceMetrics.layout_confidence),
            subtext: 'Average confidence across detected regions'
        });
    }

    if (invoiceMetrics.ocr_precision !== undefined) {
        cards.push({
            label: 'OCR Precision (invoice)',
            value: formatPercent(invoiceMetrics.ocr_precision),
            subtext: 'Parsed line items vs. detected text rows'
        });
    }

    if (running.layout_confidence_avg !== undefined) {
        cards.push({
            label: 'Layout Confidence (avg)',
            value: formatPercent(running.layout_confidence_avg),
            subtext: 'Rolling average since startup'
        });
    }

    if (running.ocr_precision_avg !== undefined) {
        cards.push({
            label: 'OCR Precision (avg)',
            value: formatPercent(running.ocr_precision_avg),
            subtext: 'Rolling average since startup'
        });
    }

    if (training.map50 !== undefined) {
        cards.push({
            label: 'YOLO mAP@50',
            value: formatPercent(training.map50),
            subtext: training.timestamp ? `Trained ${new Date(training.timestamp).toLocaleDateString()}` : 'Latest training run'
        });
    }

    if (training.map5095 !== undefined) {
        cards.push({
            label: 'YOLO mAP@50-95',
            value: formatPercent(training.map5095),
            subtext: 'COCO-style averaged IoU'
        });
    }
